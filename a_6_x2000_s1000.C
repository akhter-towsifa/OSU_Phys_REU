...
//This code finds the large jet and Higgs Boson, S masses using the bbww_x2000_s1000 file (large jet: 1H, >1 W's)

void a_6_x2000_s1000(){
  ...
  auto h_W1 = new TH1F("M_{inv} W1", "Invariant Mass of first W Boson from large jet; Mass [MeV]", nbins, 0.0, 8.0e5);
  auto h_W2 = new TH1F("M_{inv} W2", "Invariant Mass of second W Boson from large jet; Mass [MeV]", nbins, 0.0, 8.0e5);
  ...
  auto file = TFile::Open("bbww_x2000_s1000.root");
  ...
  for (int i=0; i<nentries; i++)			//loops over all the 20,000 entries.
  {
	...
	S = W1 + W2;
	h_S->Fill(S.M(), w);
	if(S.M() > 950e3 && S.M() < 1050e3) h_S_totalnumber->Fill(S.M(), w);
	
      }
    }  
    X = H + S;
    h_X->Fill(X.M(), w);
    if (X.M() > 1950e3 && X.M() < 2050e3) h_X_totalnumber->Fill(X.M(), w);
    
  } 
  
  file->Close();
  TFile f("a_6.root", "recreate");
