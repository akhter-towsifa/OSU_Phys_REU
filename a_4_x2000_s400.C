...
void a_4_x2000_s400(){
  ...
  auto h_W = new TH1F("M_{inv} W", "Invariant Mass of W Boson from large jet; Mass [MeV]", nbins, 0.0, 5.0e5);
  auto h_S = new TH1F("M_{inv} S", "Invariant Mass of S from Ws; Mass [MeV]", nbins, 0.0, 5.0e5);
  ...
  auto file = TFile::Open("bbww_x2000_s400.root");
  ...
  for (int i=0; i<nentries; i++)			//loops over all the 20,000 entries.
  {
	...
	else if (/*ljet.M() > 230e3 && ljet.M() < 250e3 && */nbtag == 0) 
	{
	  ...
	  h_W->Fill(S.M(), w);
	  if (ljet.M() > 370e3 && ljet.M() < 410e3) h_S->Fill(S.M(), w);
	}
      }
    }  
    X = H + S;
    h_X->Fill(X.M(), w);
    if (X.M() > 1850e3 && X.M() < 2050e3) h_X_number->Fill(X.M(), w);
  } 
  
  file->Close();
  TFile f("a_4.root", "recreate");
